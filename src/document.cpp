...
void shiftPendingIndices(std::vector<std::size_t>& pending, std::size_t from,
                         std::ptrdiff_t delta) {
    for (auto& index : pending)
        if (index > from || (delta > 0 && index == from))
            index = static_cast<std::size_t>(static_cast<std::ptrdiff_t>(index) + delta);
}
...
void Document::deleteModels(const std::vector<std::size_t>& indices) {
    ...
    for (const auto index : valid) {
        ...
        shiftPendingIndices(pendingIndexRebuild_, index, -1);
    }
...
void Document::replaceModel(std::size_t index, std::vector<WireframeModel> replacements) {
    ...
    shiftPendingIndices(pendingIndexRebuild_, index, delta);
    pendingIndexRebuild_.push_back(index);
    spatialIndexDirty_ = true;
    documentBounds_.reset();
}
