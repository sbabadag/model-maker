...
enum CommandId {
    ...
    CmdTrim, CmdExtend, CmdNeutral, CmdFillet, CmdRotate,
...
LRESULT Application::handleCanvasMessage(UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
        ...
        else if (wParam == 'R' && mode_ != EditMode::View3D) startTransformCommand(TransformCommand::Rotate);
...
void Application::onViewCubePaint() {
    ...
    RECT client{};
    GetClientRect(viewCube_, &client);
    viewCubeRenderer_.draw(dc, client, viewCubeCursor_, camera_);
...
void Application::exportOpenSees() {
    ...
    try {
        ...
        // Boundary conditions - fix supports at Z=0
        output << "# Boundary conditions (fixed at Z=0)\n";
        for (const auto& j : joints)
            if (std::abs(j.z) < 1e-9)
...
void Application::analyzeOpenSees() {
    ...
    output << "\n";
    // Supports: fix nodes at the lowest Y (Y-up convention); tolerance 1e-3
    for (const auto& j : joints)
        if (std::abs(j.y - minY) < 1e-3) output << "fix " << j.id << " 1 1 1 1 1 1\n";
    output << "\nset g 9.81\npattern Plain 1 Linear {\n";
    for (const auto& f : frames) {
        std::string s = f.sec.empty() || f.sec == "None" ? "None" : f.sec;
    ...
    std::wstring cmd = L"\"" + openseesPath.wstring() + L"\" \"" + tclPath.wstring() + L"\"";
